package com.example.ehirplakakodubulma

import android.os.Bundle
import androidx.appcompat.app.AppCompatActivity
import com.example.ehirplakakodubulma.databinding.ActivityMainBinding

class MainActivity : AppCompatActivity() {
    lateinit var binding: ActivityMainBinding

    // Türkiye'nin tüm illerini ve plaka kodlarını içeren harita
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.button.setOnClickListener {
            val plakaKodu = binding.editTextNumber.text.toString().toIntOrNull()
            if (plakaKodu != null) {
                val sehir = plakaKodlari[plakaKodu]
                if (sehir != null) {
                    binding.textView.text = sehir
                } else {
                    binding.textView.text = "Maalesef Bulunamadı"
                }
            } else {
                binding.textView.text = "Geçersiz Plaka Kodu"
            }
        }
    }
}
